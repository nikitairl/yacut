...
def test_create_empty_body(client):
    try:
        ...
        raise AssertionError('Если в теле запроса не передана информация - вызовите исключение.')
    assert got.status_code == 400, (
        'В ответ на пустой POST-запрос к эндпоинту `/api/id/` должен вернуться статус-код 400.'
    )
    assert list(got.json.keys()) == ['message'], (
        'В ответе на пустой POST-запрос к эндпоинту `/api/id/` должен быть ключ `message`'
    )
...
def test_invalid_short_url(json_data, client):
    ...
    assert not unique_id, (
        'В короткой ссылке должно быть разрешено использование строго определённого набора '
        'символов. Обратитесь к тексту задания.'
    )
...
def test_no_required_field(client):
    ...
    assert got.status_code == 400, (
        'Если тело запроса к эндпоинту `/api/id/` отличается от ожидаемого - верните '
        'статус-код 400.'
    )
    assert list(got.json.keys()) == ['message'], (
        'Если тело запроса к эндпоинту `/api/id/` отличается от ожидаемого - '
...
def test_get_url_endpoint(client, short_python_url):
    ...
    assert got.status_code == 200, (
        'В ответе на GET-запрос к эндпоинту `/api/id/<short_id>/` должен вернуться статус-код 200'
    )
    assert list(got.json.keys()) == ['url'], (
        'В ответе на GET-запрос к эндпоинту `/api/id/<short_id>/` должен быть передан ключ `url`'
    )
    assert got.json == {'url': py_url}, (
        'При GET-запросе к эндпоинту `/api/id/<short_id>/` возвращается ответ, '
...
def test_get_url_not_found(client):
    ...
    assert list(got.json.keys()) == ['message'], (
        'В ответе на GET-запрос для получения несуществующей ссылки должен быть '
        'передан ключ `message`'
    )
...
def test_len_short_id_api(client):
    ...
    assert got.status_code == 400, (
        ...
        'поле `short_id` содержит строку длиннее 16 символов - нужно вернуть статус-код 400.'
    )
    assert list(got.json.keys()) == ['message'], (
        'Если при POST-запросе к эндпоинту `/api/id/` '
        'поле `short_id` содержит строку длиннее 16 символов - в ответе должен быть '
        'ключ `message`.'
    )
    assert got.json == {'message': 'Указано недопустимое имя для короткой ссылки'}, (
        'При POST-запросе к эндпоинту `/api/id/`, в поле `short_id` которого передана '
        'строка длиннее 16 символов, '
        'возвращается ответ, не соответствующий спецификации.'
    )
